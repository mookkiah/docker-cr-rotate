...
def main():
        ...
        salt_code = ''
        bind_password = ''
        # Function to copy files from source to destenation
        ...
        for oxtrust_container in oxtrust_containers:
                ...
                if ''.join(salt_list).find('=') >= 0 : salt_code = salt_list[salt_list.index('=') + 1]
                # Currently print but needs to be appended  to the oxtrust log file
                else : print " Encoded salt cannot be found"
                ...
                # Currently print but needs to be appended to the oxtrust log file
                else : print "Bind Password cannot be found"
        if bind_password :
                # Return oxtrust server DN
                server_dn = ldap_containers[0].exec_run('/opt/opendj/bin/ldapsearch -h localhost -p 1636 -Z -X -D "cn=directory manager" -w ' + str(bind_password) + ' -b "ou=appliances,o=gluu"  "inum=*" | grep dn)').output.strip()
                # Return oxtrust conf cache refresh
                oxtrust_conf_cache_refresh = ldap_containers[0].exec_run('/opt/opendj/bin/ldapsearch -h localhost -p 1636 -Z -X -D "cn=directory manager" -w ' + str(bind_password) + ' -b "o=gluu" -T "objectClass=oxTrustConfiguration" oxTrustConfCacheRefresh \ | grep "^oxTrustConfCacheRefresh"').output.strip()
                # Get the currently set ip in ldap
                current_ip_in_ldap = ldap_containers[0].exec_run('/opt/opendj/bin/ldapsearch -h localhost -p 1636 -Z -X -D "cn=directory manager" -w ' + str(bind_password) + ' -b "ou=appliances,o=gluu"  "gluuIpAddress=*" gluuIpAddress \ | grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}"').output.strip()
                ...
                conf_dn = oxtrust_conf_cache_refresh[oxtrust_conf_cache_refresh.find("dn:"):oxtrust_conf_cache_refresh.find("oxTrustConfCacheRefresh")].strip()
                # Returns an index number if -1 disabled and if => 0 enabled
                is_cr_enabled = ldap_containers[0].exec_run('/opt/opendj/bin/ldapsearch -h localhost -p 1636 -Z -X -D "cn=directory manager" -w ' + str(bind_password) + ' -b "ou=appliances,o=gluu" "gluuVdsCacheRefreshEnabled=*" gluuVdsCacheRefreshEnabled \ | grep -Pzo "enabled"').output.find("enabled")
                ...
                for container in oxtrust_containers:
                        ...
                        ip = low_client.inspect_container(container.id)['NetworkSettings']['Networks'][first_default_network_name]['IPAddress'].strip()
                        ...
                        if ip != current_ip_in_ldap and is_cr_enabled >= 0 :
                                ...
                                ldap_modify_status = ldap_containers[0].exec_run('/opt/opendj/bin/ldapmodify -D "cn=directory manager" -w ' + bind_password + ' -h localhost -p 1636 --useSSL --trustAll -f ' + directory + filename).output
                                # Currently print but needs to be appended to the oxtrust log file
